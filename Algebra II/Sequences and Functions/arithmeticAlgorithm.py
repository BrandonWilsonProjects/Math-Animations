...
class ArithmeticSequenceNoLatex(Scene):
    def construct(self):
        # Title
        title = Text("Arithmetic Sequence", font_size=48, color=YELLOW).to_edge(UP)
        self.play(Write(title))

        # Formula (plain Text only)
        formula = Text("General Formula: an = a1 + (n - 1) * d", font_size=32, color=BLUE)
        ...
        example = Text(f"Example: a1 = {a1}, d = {d}", font_size=28, color=WHITE).next_to(formula, DOWN)
        ...
        for i in range(num_ticks):
            ...
            value = int(i * 2)  # spacing of 2 units
            label = Text(str(value), font_size=20, color=WHITE).next_to(tick, DOWN)
            self.play(Create(tick), Write(label))

        # Sequence points (first 6 terms)
        sequence_points = []
        for n in range(1, 7):
            an = a1 + (n - 1) * d  # arithmetic term
            # map values 0–20 into the line
            ...
            if n % 2 == 0:
                ...
                label = Text(label_text, font_size=20, color=RED).next_to(dot, DOWN)
        ...
        # Closing remark
        conclusion = Text(
            "Each term is found by adding d to the previous term",

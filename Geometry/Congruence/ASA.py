...
class ASACongruency(Scene):
    def construct(self):
        title = Text("ASA Congruency: Angle-Side-Angle", font_size=36).to_edge(UP)
        ...
        # Highlight included side
        side1 = Line(tri1.get_vertices()[0], tri1.get_vertices()[1], color=YELLOW)
        side2 = Line(tri2.get_vertices()[0], tri2.get_vertices()[1], color=YELLOW)
        self.play(Create(side1), Create(side2))

        # Larger arcs at both ends of the side

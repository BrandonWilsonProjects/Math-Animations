from manim import *

class ScaleneTriangle(Scene):
    def construct(self):
        # Title
        title = Text("Scalene Triangle", font_size=42)
        self.play(Write(title))
        self.wait(1)
        self.play(title.animate.to_edge(UP))

        # Triangle (all sides different)
        triangle = Polygon(
            [-2, -1, 0], [0.5, 2, 0], [2, -1.2, 0],
            color=RED, fill_opacity=0.5
        )
        self.play(Create(triangle))
        self.wait(1)

        label = Text("All sides and angles are different", font_size=28).next_to(triangle, DOWN)
        self.play(Write(label))
        self.wait(1)

        # Vertices
        v_left, v_top, v_right = triangle.get_vertices()

        # Draw and color each side differently
        left_side = Line(v_left, v_top, color=BLUE)
        right_side = Line(v_right, v_top, color=GREEN)
        base = Line(v_left, v_right, color=YELLOW)
        self.play(Create(left_side), Create(right_side), Create(base))
        self.wait(1)
        
        r = 0.3  # arc radius
        eps = 0.08  # tiny offset to prevent overlap

        # ✅ Angles inside the triangle (no vertex arg)
        left_angle = Angle.from_three_points(
            v_right + DOWN * 0.05,
            v_left,
            v_top + LEFT * 0.05,
            radius=0.3,
            color=RED,
            stroke_width=5,
        )

        # Right base angle ∠top-right-left (NOTE reversed order fixes orientation)
        right_angle = Angle.from_three_points(
            v_top + RIGHT * eps,
            v_right,
            v_left + DOWN * eps,
            radius=r,
            color=RED,
            stroke_width=5,
        )

        # Top angle ∠left-top-right
        top_angle = Angle.from_three_points(
            v_left + UP * eps,
            v_top,
            v_right + UP * eps,
            radius=r,
            color=RED,
            stroke_width=5,
        )

        self.play(Create(left_angle), Create(top_angle), Create(right_angle))
        self.wait(1)

        # Summary
        summary = Text(
            "A scalene triangle has NO equal sides and NO equal angles.",
            font_size=30
        ).to_edge(DOWN)
        self.play(Write(summary))
        self.wait(3)
